...
public class Cart {
	...
	// 카트에 해당 제품을 담을 때 제품이 존재하는지 아닌지 검사하는 메소드
	public static VO findProduct(String idx) {
		...
		while(it.hasNext()) {
			VO vo = (VO) it.next();
			if(vo.getIdx().equalsIgnoreCase(idx)) { // idx같은게 있으면 cvo에 담는다
				cvo = vo;
				break;
			}
		}
		return cvo; // 같은 idx가 없으면 cvo 값에 null이 담기게 됨
	}
	...
	public static void addProduct(String idx) {
		...
		if(vo == null) {
			...
			vo = DAO.getOneList(idx); // DB에서 가져오고
			vo.setQuant(1);	// 카트에 존재하는 제품의 수량
			cartList.add(vo); // 카트 리스트에 추가 
		}else {
			// 카트에 존재하는 제품의 수량을 꺼내서 1증가
